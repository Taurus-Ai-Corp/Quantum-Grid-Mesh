...
const STATUS_STYLES: Record<string, { label: string; color: string; bg: string }> = {
  draft: { label: 'Draft', color: 'text-graphite-med', bg: 'bg-bone' },
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 h-10 px-4 text-sm font-semibold text-white bg-accent rounded-brand hover:bg-accent-dark transition-colors"
        >
            ...
            <tbody>
              {assessments.map((assessment) => {
                const riskStyle = assessment.riskLevel
                  ? (RISK_STYLES[assessment.riskLevel] ?? { label: assessment.riskLevel, color: 'text-graphite-med' })
                  : null
                return (
                  <tr
                    key={assessment.id}
                  ...
                  >
                    <td className="px-5 py-4">
                      <div className="font-medium text-graphite">{assessment.systemName}</div>
                    </td>
                    <td className="px-5 py-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-brand shadow-xl border border-graphite-ghost w-full max-w-md p-6">
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 className="font-semibold text-base text-graphite">Start Assessment</h2>
                <p className="text-xs text-graphite-med">Select a system to assess</p>
              </div>
            </div>

            {systems.length === 0 ? (
              <div className="text-sm text-graphite-med mb-5">
                No AI systems registered yet.{' '}
                <Link href="/dashboard/systems/new" className="text-accent hover:underline font-medium">
                  Register one first.
                </Link>
              </div>
            ) : (
              <div className="space-y-2 mb-5">
                {systems.map((system) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 rounded-brand border text-sm transition-colors ${
                      selectedSystemId === system.id
                        ? 'border-accent bg-accent-light text-graphite'
                        : 'border-graphite-ghost hover:border-accent hover:bg-bone text-graphite'
                    }`}
                  >
                    <div className="font-medium">{system.name}</div>
                    {system.description && (
                      <div className="text-xs text-graphite-light mt-0.5 truncate">
                        {system.description}
                      </div>
                    )}
                  </button>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex-1 h-10 text-sm font-semibold text-white bg-accent rounded-brand hover:bg-accent-dark disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
